         ...
         <div class="table-responsive card-body">
            <form method="post" action="<?= base_url('experiences/store') ?>">
               ...
               <div class="mb-3">
                   ...
                   <input type="text" class="form-control" id="role" value="<?= set_value('role') ?>" name="role" required placeholder="">
                   <?= isset($error['role']) ? $error['role'] : ''  ?>
               </div>
               <div class="mb-3">
                  <label for="">Tanggal Bekerja</label>
                  <div class="input-group mb-3">
                     <input type="month" class="form-control" max="<?= date("Y-m") ?>" name="start_at" id="start_at">
                     <span class="input-group-text">s/d</span>
                     <input type="month" max="<?= date("Y-m") ?>" class="form-control" name="end_at" id="end_at">
                     <span class="input-group-text">
                        <label for="now">
                           <input type="radio" name="now" id="now" value="now"> Saya Masih Bekerja
                        </label>
                     </span>
                  </div>
