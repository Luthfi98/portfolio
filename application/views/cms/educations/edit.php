   ...
   <div class="container-fluid">
      <div class="card">
         ...
         <div class="table-responsive card-body">
            <form method="post" action="<?= base_url('educations/update/'.encrypt_decrypt('encrypt', $education->id)) ?>">
                <div class="mb-3">
                   <label for="name" class="form-label">Nama Universitas / Sekolah</label>
                   <input type="hidden" name="<?= $this->security->get_csrf_token_name();?>" value="<?= $this->security->get_csrf_hash();?>" />
                   <input type="text" class="form-control" id="name" value="<?= set_value('name', $education->name) ?>" name="name" required placeholder="">
               ...
               <div class="row">
                  <div class="mb-3 col-lg-2 col-12">
                      ...
                      <select  name="level" id="level" required data-placeholder="Pilih Tingkat Pendidikan" class="form-control select2">
                           ...
                           <option value="SMP/MTS" <?= $education->level == 'SMP/MTS' ? 'selected' : '' ?>>SMP/MTS</option>
                           <option value="SD/MI" <?= $education->level == 'SD/MI' ? 'selected' : '' ?>>SD/MI</option>
                      </select>
                      <?= isset($error['level']) ? $error['level'] : ''  ?>
                  </div>
                   <div class="mb-3 col-lg-4 col-6">
                      <label for="major" class="form-label">Jurusan</label>
                      <input type="text" class="form-control" id="major" value="<?= set_value('major', $education->major) ?>" name="major" required placeholder="">
                      <?= isset($error['major']) ? $error['major'] : ''  ?>
                  </div>

                  <div class="mb-3 col-lg-4 col-6">
                      <label for="title" class="form-label">Gelar</label>
                      <input type="text" class="form-control" id="title" value="<?= set_value('title', $education->title) ?>" name="title" placeholder="">
                      <?= isset($error['title']) ? $error['title'] : ''  ?>
                  </div>

                  <div class="mb-3 col-lg-2 col-6">
                      <label for="ipk" class="form-label">IPK/GPA</label>
                      <input type="text" class="form-control" id="ipk" value="<?= set_value('ipk', $education->ipk) ?>" name="ipk" placeholder="">
               ...
               <div class="mb-3">
                  ...
                  <div class="input-group mb-3">
                     <input type="month" class="form-control" value="<?= set_value('in', $education->in) ?>" max="<?= date("Y-m") ?>" name="in" id="in">
                     <span class="input-group-text">s/d</span>
                     <input type="month" max="<?= date("Y-m") ?>" class="form-control" value="<?= set_value('out', $education->out) ?>" name="out" id="out">
