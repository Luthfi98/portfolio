         ...
         <div class="table-responsive card-body">
            <form method="post" action="<?= base_url('projects/update/'.encrypt_decrypt('encrypt', $project->id)) ?>" enctype="multipart/form-data">
               ...
               <div class="mb-3">
                  ...
                  <select name="type" id="type" required class="form-control select2">
                     <?php foreach ($type as $value): ?>
                        <option value="<?= $value->type ?>" <?= $project->type == $value->type ? "selected" : ""  ?>><?= $value->type ?></option>
                     <?php endforeach ?>
                  </select>
               ...
               <div class="mb-3">
                   ...
                   <textarea class="form-control" id="description" name="description"><?= set_value('description', $project->description) ?></textarea>
                   <?= isset($error['description']) ? $error['description'] : ''  ?>
               </div>
               <div class="mb-3">
                   <label for="image" class="form-label">Gambar</label>
                   <div id="show-image" class="mb-3">
                   ...
                   <input type="file" accept="image/*" class="form-control" id="image" value="<?= set_value('image') ?>" name="image" placeholder="">
