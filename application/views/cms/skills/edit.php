         ...
         <div class="table-responsive card-body">
            <form method="post" action="<?= base_url('skills/update/'.encrypt_decrypt('encrypt', $skill->id)) ?>">
               ...
               <div class="mb-3">
                   ...
                   <input type="range" class="form-range" min="0" max="100" onchange="updateTextInput(this.value);" step="0.5" id="percentage" value="<?= set_value('percentage',$skill->percentage) ?>" name="percentage" required placeholder="">
                   <span id="textInput"><?= $skill->percentage ?></span>%
               ...
               <div class="mb-3">
                  ...
                  <select name="level" id="level" required class="form-control select2">
                     <?php foreach ($level as $value): ?>
                        <option value="<?= $value->level ?>" <?= $skill->level == $value->level ? "selected" : ""  ?>><?= $value->level ?></option>
                     <?php endforeach ?>
                  </select>
